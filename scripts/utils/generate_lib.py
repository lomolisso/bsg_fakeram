...
def _lib_pin_setup_hold(LIB_file, name, pin_name, slew_indicies, tsetup, thold, pindynamic):
    ...
    LIB_file.write('        capacitance : 0.000;\n')
    for timing_type, value in [('setup_rising', tsetup), ('hold_rising', thold)]:
        LIB_file.write('        timing() {\n')
        LIB_file.write('            related_pin : clk;\n')
        LIB_file.write('            timing_type : %s ;\n' % timing_type)
        for edge in ('rise', 'fall'):
            LIB_file.write('            %s_constraint(%s_constraint_template) {\n' % (edge, name))
            LIB_file.write('                index_1 ("%s");\n' % slew_indicies)
            LIB_file.write('                index_2 ("%s");\n' % slew_indicies)
            LIB_file.write('                values ( \\\n')
            LIB_file.write('                  "%.3f, %.3f", \\\n' % (value, value))
    ...
    for edge in ('rise', 'fall'):
        LIB_file.write('            %s_power(%s_energy_template_sigslew) {\n' % (edge, name))
        LIB_file.write('                index_1 ("%s");\n' % slew_indicies)
...
def _lib_bus_setup_hold(LIB_file, name, bus_name, bus_type, slew_indicies, tsetup, thold, pindynamic, min_driver_in_cap):
    '''Write setup/hold timing arcs and internal power for an input bus.'''
    LIB_file.write('    bus(%s)   {\n' % bus_name)
    LIB_file.write('        bus_type : %s;\n' % bus_type)
    LIB_file.write('        direction : input;\n')
...
def _lib_write_1rw_pins(LIB_file, name, num_rwport, slew_indicies, load_indicies,
    ...
    _lib_pin_setup_hold(LIB_file, name, 'ce_in', slew_indicies, tsetup, thold, pindynamic)

    for _ in range(int(num_rwport)):
        _lib_bus_setup_hold(LIB_file, name, 'addr_in', '%s_ADDRESS' % name,
                            slew_indicies, tsetup, thold, pindynamic, min_driver_in_cap)
...
def _lib_write_1r1w_pins(LIB_file, name, slew_indicies, load_indicies,
    ...
    _lib_bus_setup_hold(LIB_file, name, 'r_addr_in', '%s_ADDRESS' % name,
                        slew_indicies, tsetup, thold, pindynamic, min_driver_in_cap)

    # Read chip enable: r_ce_in
    _lib_pin_setup_hold(LIB_file, name, 'r_ce_in', slew_indicies, tsetup, thold, pindynamic)

    # Write data: wd_in
    LIB_file.write('    bus(wd_in)   {\n')
    LIB_file.write('        bus_type : %s_DATA;\n' % name)
    LIB_file.write('        memory_write() {\n')
    LIB_file.write('            address : w_addr_in;\n')
    LIB_file.write('            clocked_on : "clk";\n')
    LIB_file.write('        }\n')
    LIB_file.write('        direction : input;\n')
    LIB_file.write('        capacitance : %.3f;\n' % min_driver_in_cap)
    ...
    # Write address: w_addr_in
    _lib_bus_setup_hold(LIB_file, name, 'w_addr_in', '%s_ADDRESS' % name,
                        slew_indicies, tsetup, thold, pindynamic, min_driver_in_cap)

    # Write chip enable: w_ce_in
    _lib_pin_setup_hold(LIB_file, name, 'w_ce_in', slew_indicies, tsetup, thold, pindynamic)

    # Write mask: w_mask_in
    LIB_file.write('    bus(w_mask_in)   {\n')
    LIB_file.write('        bus_type : %s_DATA;\n' % name)
    LIB_file.write('        memory_write() {\n')
    LIB_file.write('            address : w_addr_in;\n')
